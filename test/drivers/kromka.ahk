...
; Сравниваются две пары кромок, и не хватать может любой из них: ok
; отвечает за прежние (a, b, c), ok2 — ещё и за новые. Без прежних в
; условии отсутствующая кромка давала не провал проверки, а падение
; драйвера на a.y — целый набор обрывался на середине.
ok2 := ok && a2 && b2 && c2
Check("C4c. места в стопке не разъехались, пока слот 3 был выдвинут",
      ok2 && a2.y = a.y && b2.y = b.y && c2.y = c.y,
      ok2 ? "было " a.y "/" b.y "/" c.y ", стало " a2.y "/" b2.y "/" c2.y : "-")
Send("^!0")
Sleep(600)
