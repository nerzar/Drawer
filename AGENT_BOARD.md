...
- Аудит старых дефектов Settings (§2 `docs/05-план-работ.md`) закрыт: все 5 пунктов перепроверены на `master`, 4 уже были не багом или закрыты раньше, 1 (честность UI при partial save) оказался настоящим живым багом и исправлен — см. `docs/05-план-работ.md` за подробностями. `npm test` 76/76.
...
- ~~`blurCheckMs`... всё ещё виден в обычном UI~~ — закрыто побочным эффектом: карточка «Дополнительно» с этим полем убрана из `GeneralView.vue` целиком при замене на карточку «Localization» (2026-09-10). Поле осталось в данных/round-trip, редактировать его через Settings UI больше нельзя.
