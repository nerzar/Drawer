...
## Product contract — immutable for this task
...
2. Explicitly assess each Antigravity proposed mechanism:
   ...
   - `fae1850` in-place handle migration: is destroy/recreate-on-monitor-change semantically compatible with accepted behavior and is there code evidence it is necessary?
   - cross-monitor Show staging / WinActivate order: identify exact accepted-vs-candidate deltas, if any.
3. Re-check the focus commits `2bab75c` and `42a16ee` as possible remaining behavior/timing deltas.
4. Build a candidate fix-boundary matrix: hypothesis, evidence for, evidence against, exact touched functions/files if eventually fixed, and risk to accepted behavior.
5. Specify regression requirements that MUST preserve existing behavior and distinguish all three revisions:
   - accepted `6bfa010`;
   - failed `cd6dc00`;
...
The report must call out any claim in the prior reports that is speculative, contradicted by exact code, or not backed by runtime evidence.