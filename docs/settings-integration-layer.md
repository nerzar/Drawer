...
- `error.field` — адрес контрола для формы: по нему она открывает вкладку,
  выбирает слот и подсвечивает поле. Ошибки формы DTO адресует сам порт;
  границы значений проверяет backend, и путь называет тоже он —
  `SettingsSlotsPlan` возвращает его полем `field` рядом с `err`. Разбирать
  русский текст ответа, чтобы вычислить путь, по-прежнему нельзя: это ровно
  то, от чего уводил C3. Когда контрол назвать не из чего, поле сужается до
  слота (`slots.3`) — форма доводит до слота, а не до контрола.
- В самом `error.message` путь поля не появляется: там стоит название
  контрола («слот 3, размер окна»). Имена полей слота живут в одном месте —
  `SettingsSlotFieldLabel()`, и порт берёт их оттуда же, откуда backend
  берёт подписи для своих сообщений о границах.
